...
namespace ORB::Math {
	...
	{
		// From glm::decompose in matrix_decompose.h

		using namespace glm;
		using T = float;

		m4 LocalMatrix(transform);
		...
		outTranslation = v3(LocalMatrix[3]);
		LocalMatrix[3] = v4(0, 0, 0, LocalMatrix[3].w);
	...
	#if 0
		Pdum3 = cross(Row[1], Row[2]); // v3Cross(row[1], row[2], row[3]);
