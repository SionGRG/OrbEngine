...
#include <filesystem>
	...
	#define ORBE_INTERNAL_ASSERT_NO_MSG(type, check) ORBE_INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}", ORBE_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).filename().string(), __LINE__)
