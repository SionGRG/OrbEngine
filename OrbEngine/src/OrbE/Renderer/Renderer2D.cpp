...
namespace ORB {
	...
	void Renderer2D::DrawLine(const v3& p0, const v3& p1, const v4& color, int entityID)
	{
	...
	{
		v3 p0 = v3(position.x - size.x * 0.5f, position.y - size.y * 0.5f, position.z);
		v3 p1 = v3(position.x + size.x * 0.5f, position.y - size.y * 0.5f, position.z);
		v3 p2 = v3(position.x + size.x * 0.5f, position.y + size.y * 0.5f, position.z);
		v3 p3 = v3(position.x - size.x * 0.5f, position.y + size.y * 0.5f, position.z);
	...
	void Renderer2D::DrawRect(const m4& transform, const v4& color, int entityID)
	{
		v3 lineVertices[4];
		for (size_t i = 0; i < 4; i++)
