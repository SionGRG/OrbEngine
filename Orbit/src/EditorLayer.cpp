...
namespace ORB {
	...
	void EditorLayer::OnAttach()
	{
		...
		auto commandLineArgs = App::Get().GetSpecification().CommandLineArgs;
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		// Draw an outline on selected entity
		if (Entity selectedEntity = m_SceneHierarchyPanel.GetSelectedEntity())
		{
			const TransformComponent& transform = selectedEntity.GetComponent<TransformComponent>();
			Renderer2D::DrawRect(transform.GetTransform(), glm::vec4(0.0f, 1.0f, 1.0f, 1.0f));
		}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".orb")
		{
			ORBE_WARN("Could not load '{0}' - not a scene file!", path.filename().string());
			return;
		}
	...
	{
		ORBE_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
