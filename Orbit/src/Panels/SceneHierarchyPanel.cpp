...
namespace ORB {
	...
	template<typename T>
	inline void ORB::SceneHierarchyPanel::DisplayAddComponentEntry(std::string_view entryName)
	{
	...
	void SceneHierarchyPanel::DrawComponents(Entity entity)
	{
		...
		if (ImGui::BeginPopup("AddComponent"))
		{
			...
			DisplayAddComponentEntry<CircleCollider2DComponent>("Circle Renderer");
				...
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
				{
					...
					if (Utils::HasSuffix(wstr, suffixes))
					{
						...
						if (texture->IsLoaded())
							...
							ORBE_WARN("Could not load texture {0}", texturePath.filename().string());
					}
				}
